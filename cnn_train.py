...
class GraspTrain(object):
    def __init__(self, cnn_policy_model, cnn_value_model, action_dim, state_dim, lr=0.0001, update_steps=4, buff_capability=32, batch_size=32, gamma=0.9, ppo_epoch=10):

        self.cnn_policy = cnn_policy_model(action_dim).cuda()
        self.cnn_old_policy = cnn_policy_model(action_dim).cuda()
        ...
        self.writer = SummaryWriter('./logs_grasp_cnn')
    ...
    def learn(self):
        ...
        self.cnn_old_policy.load_state_dict(self.cnn_policy.state_dict())
        ...
        with torch.no_grad():
            old_mean, old_std = self.cnn_old_policy(ob, pos)
        ...
        for _ in range(self.ppo_epoch):
            ...
            value_loss = F.mse_loss(self.cnn_value(ob, pos), target_v)

            self.cnn_policy_optim.zero_grad()
            action_loss.backward()
            self.cnn_policy_optim.step()
            self.cnn_value_optim.zero_grad()
            value_loss.backward()
            self.cnn_value_optim.step()

            self.writer.add_scalar('action loss', action_loss.item(), self.loss_mlp_steps)
            self.writer.add_scalar('value loss', value_loss.item(), self.loss_mlp_steps)
            #
    ...
    def save_model(self, cnn_policy_path, cnn_value_path):
    ...
    def load_model(self, cnn_policy_path, cnn_value_path):
        save_path_cnn = '/home/chen/PycharmProjects/Reinforcement/VisualGrasp/model/' + cnn_policy_path
        save_value_cnn = '/home/chen/PycharmProjects/Reinforcement/VisualGrasp/model/' + cnn_value_path
